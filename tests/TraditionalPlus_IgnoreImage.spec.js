...
test('Traditional Plus job >> Ignore Team Images ', async ({ page, request }) => {
  ...
  await allure.step('Handle Traditional Plus selection and popups', async () => {
    ...
    // ===== POPUP 2: "You have images in the image5 column. Please ignore Team Images." =====
    try {
      await page.waitForSelector('text=You have images in the image5 column', { timeout: 12000 });
      const ignoreBtn = page.locator('button:has-text("Ignore Team Images")');
      if (await ignoreBtn.count() > 0) {
        await ignoreBtn.first().click();
        console.log('✔ Clicked Ignore Team Images');
      } else {
        // some apps put the ignore button as input or link — try generic
        await page.click('text=Ignore Team Images', { timeout: 8000 });
        console.log('✔ Clicked Ignore Team Images (text click)');
      }
    } catch (e) {
