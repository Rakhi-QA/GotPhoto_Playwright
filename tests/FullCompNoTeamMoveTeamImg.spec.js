...
test('Full Composite NO Team Move >> Place Order + FTP Upload + Confirm Image Transfer', async ({ page, request }) => {
  ...
  // ============================================================
  // ========== HANDLE TEAM IMAGES POPUP (MOVE) =================
  // ============================================================

  await allure.step('Handle Team Images Popup (Move Team Images)', async () => {
    try {
      await page.waitForSelector(
        'text=images present in the Team Image column',
        { timeout: 15000 }
      );
      ...
      if (await moveBtn.isVisible()) {
        console.log("✔ Popup detected: Clicking Move Team Images");
        await moveBtn.click();
      }

      // Wait for the page refresh/load after moving images
