...
test('Traditional job >> Move Team Images (Full flow)', async ({ page, request }) => {
  // ================================
  // ✅ METADATA
  // ================================
  ...
  await allure.step('Select Traditional job and handle popups', async () => {
    ...
    // ===== POPUP 2: Click "Move team images" button =====
    try {
      await Promise.race([
        ...
        page.waitForSelector('#moveTeamImages', { timeout: 8000 }).catch(() => {})
      ]);

      // Prefer button with text "Move team image(s)" or id #moveTeamImages
      const moveBtnSelectors = [
        '#moveTeamImages',
        'button:has-text("Move team images")',
        'button:has-text("Move team image")',
        '[id="moveTeamImages"]',
        '.bootbox .modal-footer button:has-text("Move team images")',
        '.bootbox .modal-footer button:has-text("Move team image")',
        'text=Move team images',
        'text=Move team image'
      ];
      let clickedMove = false;
      for (const sel of moveBtnSelectors) {
        try {
          const btn = page.locator(sel).first();
          if (await btn.isVisible({ timeout: 3000 }).catch(() => false)) {
            await btn.scrollIntoViewIfNeeded();
            await btn.click({ force: true, timeout: 5000 });
            console.log('✔ Clicked Move team images button:', sel);
            clickedMove = true;
            break;
          }
        } catch (e) {
      ...
      if (!clickedMove) {
        const moveBtn = page.locator('.bootbox .modal-footer button').filter({ hasText: /Move\s*team\s*image/i });
        if ((await moveBtn.count()) > 0) {
          await moveBtn.first().click({ force: true });
          console.log('✔ Clicked Move team images (bootbox filter)');
        } else {
          console.warn('⚠ Move team images button not found by any strategy');
        }
      }
    } catch (e) {
      console.warn('⚠ Second popup (Move team images) not found or not clickable:', e.message);
    }
